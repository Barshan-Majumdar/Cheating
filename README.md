...
- **wkhtmltopdf**: Required for PDF report generation. [Download here](https://wkhtmltopdf.org/downloads.html).
- **Webcam & Microphone**: Standard hardware for monitoring.
...
Problem: Object detection (mobile phone, book) is not accurate.

Solution:
    - Ensure the object is clearly visible to the camera.
    - Improve lighting in the room to reduce shadows.
    - Adjust `min_confidence` in `config/config.yaml` to be lower (e.g., 0.5) if detection is too strict.
    - Note: This system uses a lightweight YOLOv8 nano model for speed; for better accuracy, consider using a larger model if hardware allows.
